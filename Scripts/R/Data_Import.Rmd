...
#qiime2R is used to generate aphyloseq object from the exported files
Euk.18S.data_SEQ033_rare <- qiime2R::qza_to_phyloseq(features="../../Data/18S/SEQ033/rarefied-table.qza",
                                      taxonomy="../../Data/18S/SEQ033/taxonomy.qza",
                              metadata="../../Data/18S/SEQ033/metadata_Rinput.tsv")

#qiime2R is used to generate aphyloseq object from the exported files
Euk.18S.data_SEQ081_rare <- qiime2R::qza_to_phyloseq(features="../../Data/18S/SEQ081/rarefied-table.qza",
...
long$Cohort[long$Cohort_Short == "A"] <- "Naïve"
...
long$Cohort <- ordered(long$Cohort, levels = c("Naïve", "Exp", "HC"))
...
base$Cohort[base$Cohort_Short == "A"] <- "Naïve"
...
base$Cohort <- ordered(base$Cohort, levels = c("Naïve", "Exp", "HC"))
...
####16S Microbiome data
```{r}
#### 16S Microbiome data (Synchronized Version) ####

# 1. Generate Counts and Pivot Wide
fecal.gen.count <- prep_mdf(fecal.tax.plot.data, subgroup_level="Genus", as_relative_abundance = FALSE)

fecal.gen.wide <- fecal.gen.count %>%
  group_by(Sample, Genus) %>%
  summarise(Counts = sum(Abundance), .groups = 'drop') %>%
  pivot_wider(id_cols = "Sample", names_from = "Genus", values_from = "Counts", values_fill = 0)

# 2. Filter for specific taxa (Bacteria only, no uncultured)
# We do this FIRST so all downstream versions (ct, ra, clr) match
bac_genera <- unique(fecal.gen.count$Genus[which(
  !is.na(fecal.gen.count$Genus) & 
  fecal.gen.count$Kingdom == "d__Bacteria" & 
  fecal.gen.count$Genus != "uncultured"
)])

# Subset wide data to only include these genera
fecal.gen.filtered <- fecal.gen.wide %>% select(Sample, all_of(bac_genera))
fecal.gen.names <- bac_genera # Save original names for mapping

# 3. Create the Three Versions
# --- A. Counts ---
fecal.gen.ct <- fecal.gen.filtered
colnames(fecal.gen.ct)[2:ncol(fecal.gen.ct)] <- paste0("g_16S_ct_", 1:(ncol(fecal.gen.ct)-1))

# --- B. Relative Abundance ---
fecal.gen.ra <- fecal.gen.filtered
# Divide each row by its sum (excluding Sample column)
fecal.gen.ra[,-1] <- fecal.gen.ra[,-1] / rowSums(fecal.gen.ra[,-1])
fecal.gen.ra[is.na(fecal.gen.ra)] <- 0
colnames(fecal.gen.ra)[2:ncol(fecal.gen.ra)] <- paste0("g_16S_ra_", 1:(ncol(fecal.gen.ra)-1))

# --- C. CLR ---
fecal.gen.clr <- fecal.gen.filtered
fecal.gen.clr[,-1] <- fecal.gen.clr[,-1] + 1 # Add pseudocount
fecal.gen.clr[,-1] <- compositions::clr(fecal.gen.clr[,-1]) # Transform
colnames(fecal.gen.clr)[2:ncol(fecal.gen.clr)] <- paste0("g_16S_clr_", 1:(ncol(fecal.gen.clr)-1))

# 4. Join to Base Metadata
# Ensure SampleID is the join key
fecal.gen.ct <- rename(fecal.gen.ct, SampleID = Sample)
fecal.gen.ra <- rename(fecal.gen.ra, SampleID = Sample)
fecal.gen.clr <- rename(fecal.gen.clr, SampleID = Sample)

base_with_16S_ct  <- left_join(base, fecal.gen.ct, by = "SampleID")
base_with_16S_ra  <- left_join(base, fecal.gen.ra, by = "SampleID")
base_with_16S_clr <- left_join(base, fecal.gen.clr, by = "SampleID")

# 5. Create Master Name Map
mic_name_map <- data.frame(
  Original_Name = fecal.gen.names,
  Safe_ct  = paste0("g_16S_ct_", 1:length(fecal.gen.names)),
  Safe_ra  = paste0("g_16S_ra_", 1:length(fecal.gen.names)),
  Safe_clr = paste0("g_16S_clr_", 1:length(fecal.gen.names)),
...
#Blastocytsis pulled for validation analysis
Zim_blast_sg_genera_ab <- Zim_all_sg_genera_pre[which(substr(Zim_all_sg_genera_pre$shotgunid,16,20)=="frac" & Zim_all_sg_genera_pre$genus=="Blastocystsis"),]
Zim_blast_sg_genera_ab$SampleID <- paste0(str_extract(Zim_blast_sg_genera_ab$shotgunid, "ZIM.{3}"),".2")
...
#Stratified 
Zim_all_wide_strat <- pivot_wider(Zim_all_long_strat, id_cols="Sample",names_from="Gene.Family", values_from="RPK", values_fill = 0)
#Unstratified 
Zim_all_wide_unstrat <- pivot_wider(Zim_all_long_unstrat, id_cols="Sample",names_from="Gene.Family", values_from="RPK", values_fill = 0)
...
#We pull the SampleID and from the clutted samle field 
Zim_all_wide_strat$SampleID <- paste0(str_extract(Zim_all_wide_strat$Sample, "ZIM\\d{3}"),".2")
#We pull the SampleID and from the clutted samle field 
Zim_all_wide_strat_TPM$SampleID <- paste0(str_extract(Zim_all_wide_strat_TPM$Sample, "ZIM\\d{3}"),".2")
...
#We add metadata using inner_join
base_with_sg_fun_strat <- inner_join(Zim_all_wide_strat_TPM, base, by="SampleID")
...
Euk_18S_mdf_33 <- prep_mdf(Euk.18S.data_SEQ033_euk_nonmammal, subgroup_level="Species", as_relative_abundance = FALSE)
Euk_18S_mdf_81 <- prep_mdf(Euk.18S.data_SEQ081_euk_nonmammal, subgroup_level="Species", as_relative_abundance = FALSE)
...
Euk_18S_mdf$Week <- (as.numeric(substr(Euk_18S_mdf$Sample,8,8))-2)*24


##Rarefied

#Mamallian ares removed
Euk.18S.data_SEQ033_euk_nonmammal_rare <- subset_taxa(Euk.18S.data_SEQ033_rare, Kingdom=="d__Eukaryota" & Class!="Mammalia")
Euk.18S.data_SEQ081_euk_nonmammal_rare <- subset_taxa(Euk.18S.data_SEQ081_rare, Kingdom=="d__Eukaryota" & Class!="Mammalia")

#prepMDF is used to generate a long file with species and abundances
Euk_18S_mdf_33_rare <- prep_mdf(Euk.18S.data_SEQ033_euk_nonmammal_rare, subgroup_level="Species", as_relative_abundance = FALSE)
Euk_18S_mdf_81_rare <- prep_mdf(Euk.18S.data_SEQ081_euk_nonmammal_rare, subgroup_level="Species", as_relative_abundance = FALSE)

#We commbine the tables into one
Euk_18S_mdf_rare <- rbind(Euk_18S_mdf_33_rare[,Euk_cols],Euk_18S_mdf_81_rare[,Euk_cols])

#We add sample IDs and PIDs
Euk_18S_mdf_rare$SampleID <- Euk_18S_mdf_rare$Sample
Euk_18S_mdf_rare$PID <- substr(Euk_18S_mdf_rare$Sample,1,6)
Euk_18S_mdf_rare$Week <- (as.numeric(substr(Euk_18S_mdf_rare$Sample,8,8))-2)*24
...
Ent_Blast_data$Counts_18S <- NA
Ent_Blast_data$Blastocystis <- NA
Ent_Blast_data$BlastocystisPresent <- ""
Ent_Blast_data$Blastocystis_Hominis <- NA
Ent_Blast_data$Blastocystis_sp <- NA
Ent_Blast_data$Blastocystis_rare <- NA
Ent_Blast_data$BlastocystisPresent_rare <- ""
Ent_Blast_data$BlastocystisCategory <- ""


med_Blast <- median(as.numeric(Euk_18S_mdf_rare$Abundance[which(Euk_18S_mdf_rare$Genus=="Blastocystis")]))
#Now we go through and add the abundances of each
for (j in 1:nrow(Ent_Blast_data)) {
  PID_j <- Ent_Blast_data$PID[j]
  week_j <- Ent_Blast_data$Week[j]
  mdf_data_j <- Euk_18S_mdf[which(Euk_18S_mdf$PID==PID_j & Euk_18S_mdf$Week==week_j),]
  Ent_Blast_data$Counts_18S[j] <- sum(!is.na(mdf_data_j$Abundance))
  Ent_Blast_data$Blastocystis[j] <- sum(mdf_data_j$Abundance[which(mdf_data_j$Genus=="Blastocystis")])
  if (sum(mdf_data_j$Abundance[which(mdf_data_j$Genus=="Blastocystis")])>0) {
    Ent_Blast_data$BlastocystisPresent[j] <- "Yes"
  } else {
    Ent_Blast_data$BlastocystisPresent[j] <- "No"
  }
  Ent_Blast_data$Blastocystis_Hominis[j] <- sum(mdf_data_j$Abundance[which(mdf_data_j$Species=="Blastocystis_hominis")])
  Ent_Blast_data$Blastocystis_sp[j] <- sum(mdf_data_j$Abundance[which(mdf_data_j$Species=="Blastocystis_sp.")])
  mdf_data_j_rare <- Euk_18S_mdf_rare[which(Euk_18S_mdf_rare$PID==PID_j & Euk_18S_mdf_rare$Week==week_j),]
  Ent_Blast_data$Blastocystis_rare[j] <- sum(mdf_data_j_rare$Abundance[which(mdf_data_j_rare$Genus=="Blastocystis")])
  if (sum(mdf_data_j_rare$Abundance[which(mdf_data_j_rare$Genus=="Blastocystis")])>0) {
    Ent_Blast_data$BlastocystisPresent_rare[j] <- "Yes"
  } else {
    Ent_Blast_data$BlastocystisPresent_rare[j] <- "No"
  }
  if (sum(mdf_data_j_rare$Abundance[which(mdf_data_j_rare$Genus=="Blastocystis")])>med_Blast) {
    Ent_Blast_data$BlastocystisCategory[j] <- "High"
  } else {
    Ent_Blast_data$BlastocystisCategory[j] <- "Low"
  }
}

Ent_Blast_data_add <- Ent_Blast_data[,c("SampleID","Counts_18S","Blastocystis", "BlastocystisPresent","Blastocystis_Hominis","Blastocystis_sp","Blastocystis_rare","BlastocystisPresent_rare", "BlastocystisCategory")]

base_with_18S <- inner_join(base,Ent_Blast_data_add, by="SampleID")

base_with_18S_1 <- base_with_18S[which(base_with_18S$Week==0),]
base_with_18S_2 <- base_with_18S[which(base_with_18S$Week==24),]

hdl_cor_1 <- cor.test(base_with_18S_1$Blastocystis,base_with_18S_1$HDL, method="spearman")
ldl_cor_1 <- cor.test(base_with_18S_1$Blastocystis,base_with_18S_1$LDL, method="spearman")
trig_cor_1 <- cor.test(base_with_18S_1$Blastocystis,base_with_18S_1$Trig, method="spearman")
gluc_cor_1 <- cor.test(base_with_18S_1$Blastocystis,base_with_18S_1$Glucose, method="spearman")


hdl_cor_2 <- cor.test(base_with_18S_2$Blastocystis,base_with_18S_2$HDL, method="spearman")
ldl_cor_2 <- cor.test(base_with_18S_2$Blastocystis,base_with_18S_2$LDL, method="spearman")
trig_cor_2 <- cor.test(base_with_18S_2$Blastocystis,base_with_18S_2$Trig, method="spearman")
gluc_cor_2 <- cor.test(base_with_18S_2$Blastocystis,base_with_18S_2$Glucose, method="spearman")
...
#16S Objects
  ...
  mic_16S_objects <- list(base=base,
                          base_with_16S_clr=base_with_16S_clr,
                          base_with_16S_ct=base_with_16S_ct,
                          base_with_16S_ra=base_with_16S_ra,
...
#Shotgun Functional Objects
  ...
  mic_sg_fun_objects <- list(base=base,
                             ...
                             base_with_sg_fun_strat=base_with_sg_fun_strat)
